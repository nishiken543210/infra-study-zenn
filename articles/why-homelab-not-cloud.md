...
いろいろ調べた末に選んだのは、**HPE ML30 Gen10**。エントリークラスのタワー型サーバーです。新品だと少し値が張りますが、中古市場を探すと意外と手頃な価格で見つかりました。
...
- 受講者用の「学習 VM（learner01）」と「踏み台サーバー（workstation01）」を別々に用意
...
受講者の方は、特別なソフトをインストールしなくてもブラウザだけで演習に入れます。アクセス先は `lab.infra-study.org`。認証を通過したらすぐに Linux のプロンプトが現れます。
...
このプラットフォームが対象としているのは、**IT の経験はあるけれど、実機を触ったことがない中級の学習者の方**です。

「コマンドラインは知っている。`ls` も `grep` も使える。でもサーバーの前に立つと、何をどう触ればいいのかわからない」——そういう方に使ってもらえたら嬉しいです。CCNA の資格取得とは少し違う軸で、**Linux の実機感覚、ネットワークの動作感覚を体で覚える**ことを目指しています。
...
公開目標は **2026年7月12日**。現在、Part 1（Linux 基礎 / 8章）が完成していて、Part 2（ネットワーク基礎）の執筆を進めています。
...
> **infra-study.org** は現在ベータテスター募集中です。「早めに試してみたい」という方がいらっしゃれば、X（旧 Twitter）でご連絡ください。
