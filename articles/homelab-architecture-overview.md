...
受講者がブラウザで `infra-study.org` を開く → 章の内容を読む → 「演習を始める」ボタンを押す → Guacamole が SSH 端末をブラウザ上に開く → 受講者は `workstation01` に入り、そこから `ssh learner01` でジャンプして演習できる、という流れです。
...
| 受講者演習環境 | learner01 |
| ブラウザ端末 | Guacamole |
| 踏み台サーバー | workstation01 |
| L3 ルーター演習 | l3router01 |
