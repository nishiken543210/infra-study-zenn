---
title: "PowerShell スクリプトの文字コードを26本調べてみた ── 壊れていたのは1本、直したのは9本"
emoji: "💥"
type: "tech"
...
前日まで押せていた、サーバを起動するボタンが動かなくなりました。
ボタンの先は手元の PowerShell スクリプトで、起動経路をばらばらにしないため、これだけを入口にしていました。

*「昨日まで動いていたのに、なぜ今日だけ壊れた？」*

表示されたのは見覚えのない構文エラーで、処理を変えた覚えもないのに、スクリプトを読むところで止まっています。
調べると、問題のファイル先頭に UTF-8 の BOM がありませんでした。Windows PowerShell 5.1 が CP932 として読み、日本語が化けていました。
ただ、ここで「BOM がないから壊れた」と結論にすると、今回の事故は半分しか説明できません。
同じ欠陥を持つファイルはほかにもあり、あるなら、なぜ前日に壊れた1本しか見えていなかったのでしょう。
...
この記事は、infra-study「Part 0 / Chapter 0」でLinuxの基礎を読むときの、おまけとして作成しています。
